...
//      saknas. 100 % här = självbärande kopia.
//
//   2. PIXEL-LIKHET mot live-referensen (diagnostik-golv): blockvis
...
// ── 1) MEDIA-KOMPLETTHET: fryst kopia renderad OFFLINE ───────────────────────
const media = await (async () => {
  ...
  const res = await page.evaluate(() => {
    ...
    return { rows, players, playersPainted };
  });
  await ctx.close();
  const total = res.rows.length + res.players;
  const painted = res.rows.filter((r) => r.painted).length + res.playersPainted;
  const blanks = res.rows.filter((r) => !r.painted).slice(0, 8);
  return { total, painted, blanks, players: res.players, playersPainted: res.playersPainted };
})();

// ── 2) PIXEL-LIKHET: fryst (nätverk PÅ, visningsläget) vs live-referensen ────
...
const completeness = media.total > 0 ? Math.round((media.painted / media.total) * 1000) / 10 : 100;
console.log(
  `[fidelity] MEDIA-KOMPLETTHET (offline): ${completeness}% — ${media.painted}/${media.total} media-element målar (${media.playersPainted}/${media.players} spelare med poster)`,
);
