...
export function notifyInstalled(site: string, domain: string): Promise<number> {
  return notifyOwners(
    ...
    "installed",
    domain,
    `Installationen fungerar — ${domain} skickar data`,
    [
      `Hej!`,
      ``,
      `Er snippet på ${domain} har skickat sin första signal och domänen är verifierad.`,
...
export function notifyVariantReady(site: string, variantId: string, label: string): Promise<number> {
  return notifyOwners(
    ...
    "variant_ready",
    variantId,
    `En förbättring av er sida är redo att granska`,
    [
      `Hej!`,
      ``,
      `Systemet har tagit fram och verifierat en variant av er sida (${label}).`,
